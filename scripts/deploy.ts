...
for (const contract of contracts) {
  ...
  if (existingContractIds[contract.packageName]) {
    console.log(`✅ ${contract.packageName} already deployed: ${existingContractIds[contract.packageName]}\n`);
    deployed[contract.packageName] = existingContractIds[contract.packageName];
    continue;
  }
  ...
  try {
    ...
    const deployResult =
      await $`stellar contract deploy --wasm ${contract.wasmPath} --source-account ${adminSecret} --network ${NETWORK} -- --admin ${adminAddress} --game-hub ${mockGameHubId}`.text();
